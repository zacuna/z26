...
| 2/20/2026 | Cursor | Theme Editor (Phase 3): components/theme-editor/ with ThemeEditor.tsx (right-side panel), TokenField.tsx (color dropdown with Tailwind palette + search/swatches, radius segmented control, spacing/shadow dropdowns), primitives.ts (Tailwind color palette, spacing, radius, font, shadows), export-utils.ts (JSON/CSS copy). Collapsible UI component added. /play/design-system layout: left = playground, right = Theme Editor. In-memory only; Reset and Export (no save-to-file in this pass). | Theme Editor visible to all users; changes reset on leave. |
| 2/20/2026 | Cursor | FIX 1 — Header overlap: main content uses pt-14 to offset fixed header (56px). Layout tokens removed; app-shell/side-nav/design-system use Tailwind values (pt-14, top-14, h-14, w-16/w-60, ml-16/ml-60, min-h-[calc(100vh-3.5rem)]). FIX 2 — globals.css reset to Shadcn v4 default theme (oklch, @theme inline, @layer base); re-added only custom z-index primitives. tailwind.config.ts stripped to content + plugins (no theme.extend). Site header/sidebar use z-[var(--z-header)] and z-[var(--z-sidebar)]. Design-system SPACING_SCALE uses Tailwind default px values. | Verify locally: pages render, Shadcn buttons, light/dark, no header overlap. |
| 2/20/2026 | Cursor | Theme Editor color format: primitives.ts palette converted from HSL to oklch (Tailwind v4 default from tailwindcss.com/docs/colors). TokenField.tsx: swatchStyle uses value as backgroundColor directly; getColorLabel uses normalizeOklch() for comparison (trim, collapse spaces, %→0-1 for L, 4-decimal rounding) so DOM values match palette; selection and onChange pass oklch. Design System page now shows named Tailwind colors (e.g. gray-900, neutral-950) instead of "Custom" for Shadcn defaults. | — |
| 2/20/2026 | Cursor | Shadcn: added sidebar, breadcrumb, badge, avatar, tabs, accordion, dialog, scroll-area, collapsible, sonner, skeleton, aspect-ratio. lucide-react verified. App Shell refactored to use Shadcn Sidebar: SidebarProvider + Sidebar (collapsible=icon) + SidebarInset; nav items Home, Work, Play, About, Contact with lucide icons (Home, Briefcase, Gamepad2, User, Mail). SiteHeader uses SidebarTrigger; Z26 branding and ThemeToggle unchanged. Custom side-nav.tsx removed. | Verify in browser: pages render, sidebar expand/collapse, mobile sheet. |
| | | | |
